...
use cache::{hash_file, hex_digest, marketplace_cache_dir, safe_version_component, sha256_hex};
...
pub async fn load_catalog(expected_abi: u32) -> Result<MarketplaceCatalog, String> {
    ...
    validate_catalog(document, expected_abi).await
}
...
pub async fn download_plugin(
    ...
    if destination.is_file() && hash_file(&destination)? == plugin.sha256 {
        validate_package_manifest(plugin, &destination, expected_abi)?;
    ...
    let result = download_package(plugin, &temporary)
        .await
        .and_then(|()| validate_package_manifest(plugin, &temporary, expected_abi));
...
fn validate_package_manifest(
    ...
    let manifest = zip_loader::read_manifest_from_zip(path, expected_abi)?;
